...
class Controller(nn.Module):
    ...
    def __call__(self, x, carry=None):
        ...
        if carry is None:
            carry = [cell.initialize_carry(jax.random.key(0), x.shape) for cell in cells]
...
@partial(jax.jit, static_argnames=["apply_fn", "optimizer", "T1", "T2", "lam_id_loss", "lam_ex_loss", "lam_tr_loss", "lam_st_loss"])
def update(apply_fn, params, lti: LTI, opt_state, refs, carry, optimizer, T1, T2, lam_id_loss, lam_ex_loss, lam_tr_loss, lam_st_loss):

    def loss_fn(params, refs, carry):
        def closed_loop(carry, ref_t):
            controller_carry, x_tm1, y_tm1, u_tm1, t = carry
            (u_t, yhat_t), controller_carry = apply_fn(params, jnp.concatenate([y_tm1, ref_t, jax.lax.stop_gradient(u_tm1), t], axis=-1), controller_carry)
        ...
        loss_term = id_loss * lam_id_loss + expl_loss * lam_ex_loss + track_loss * lam_tr_loss + stabl_loss * lam_st_loss
        loss_logging = {"stabl_loss": stabl_loss, "track_mae_after_40s": jnp.mean(jnp.abs(ys - refs)), "id_loss": id_loss, "expl_loss": expl_loss, "track_loss": track_loss}
        return loss_term, (carry, loss_logging)

    (loss, (carry, loss_terms)), grads = jax.value_and_grad(loss_fn, has_aux=True)(params, refs, carry)
    updates, opt_state = optimizer.update(grads, opt_state, params)
    params = optax.apply_updates(params, updates)
    return params, opt_state, loss, carry, loss_terms


def train_controller(controller, params, opt_state, F, batch_size, state_dim, steps, optimizer, T1, T2, lam_id_loss, lam_ex_loss, lam_tr_loss, lam_st_loss, logging=False, **kwargs):
    _, carry = controller.apply(params, jnp.zeros((batch_size, F)))
    x = jnp.zeros((batch_size, state_dim))
    init = (carry, x, jnp.zeros((batch_size, 1)), jnp.zeros((batch_size, 1)), jnp.zeros((batch_size, 1)))

    for step in range(steps):
        lti = generate_random_lti_systems(batch_size, state_dim, **kwargs)
        refs = lti.refs.transpose(1, 0, 2)

        params, opt_state, loss, _, loss_terms = update(controller.apply, params, lti, opt_state, refs, init, optimizer, T1, T2, lam_id_loss, lam_ex_loss, lam_tr_loss, lam_st_loss)

        if (step % 5 == 0) and logging:
            print(f"Step {step}: Loss = {jnp.mean(loss):.4f}", ', '.join([f"{key} = {float(val):.4f}" for key, val in loss_terms.items()]))
...
def objective(config):
    ...
    optimizer = optax.chain(optax.clip_by_global_norm(config["grad_clip"]), optax.adam(learning_rate=config["lr"], b1=config["b1"], b2=config["b2"], eps=config["eps"]))
    opt_state = optimizer.init(params)
    
    for step in range(max_steps):
        params, opt_state, tracking_mae = train_controller(controller, params, opt_state, F, config["batch_size"], config["state_dim"], 1, optimizer,
                                          config["T1"], config["T2"], config["lam_id_loss"], config["lam_ex_loss"], config["lam_tr_loss"], 
                                          config["lam_st_loss"], logging=False, draw_step_function_reference=config["draw_step_function_reference"])
...
tuner = tune.Tuner(
    tune.with_resources(objective, {"gpu": 0.5, "cpu": 4}), param_space=param_space, tune_config=tune.TuneConfig(
            mode="min",
            metric="tracking_mae",
